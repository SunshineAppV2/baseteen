            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-6">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">
                        Acompanhe o desempenho e pontos dos adolescentes e eventos.
                    </p>
                </div>
                ...
                <div className="flex bg-gray-100 p-1 rounded-xl">
                    <button
                        ...
                        className={clsx(
                            ...
                            activeTab === "geral" ? "bg-white text-primary shadow-sm" : "text-gray-500 hover:text-gray-700"
                        )}
                    >
                        Ranking Geral
                    </button>
                    <button
                        ...
                        className={clsx(
                            "px-6 py-2 rounded-lg text-sm font-bold transition-all",
                            activeTab === "eventos" ? "bg-white text-primary shadow-sm" : "text-gray-500 hover:text-gray-700"
                        )}
                    >
                        Rankings de Eventos
                    </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {allEvents.length === 0 ? (
                        <div className="col-span-full py-20 text-center bg-gray-50 rounded-3xl border-2 border-dashed border-gray-100">
                            ...
                            <p className="text-gray-500 font-medium">Nenhum evento com ranking disponível no momento.</p>
                        </div>
                    ) : (
                        allEvents.map((ev: any) => (
                            <div key={ev.id} className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-all group">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className={clsx(
                                        ...
                                        ev.status === 'active' ? "bg-green-100 text-green-700 border-green-200" : "bg-gray-100 text-gray-700 border-gray-200"
                                    )}>
                                        {ev.status === 'active' ? "AO VIVO" : "FINALIZADO"}
                                    </span>
                                </div>
                                <h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-primary transition-colors">{ev.title}</h3>
                                <p className="text-sm text-gray-500 mb-6 line-clamp-2">{ev.description || "Ranking oficial do evento."}</p>
                                <button
                                    onClick={() => window.open(`/events/${ev.id}/ranking`, '_blank')}
                                    className="w-full bg-gray-50 hover:bg-primary hover:text-white text-primary font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2"
                                >
